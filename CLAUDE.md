...
**Category filter is plumbed but unrendered** — `GameState.category`, `setCategory()`, the `startSession` filter and `Ui.CATEGORY_HE` (24 Hebrew theme labels) all exist and work, but **no composable calls `setCategory`**, so `category` is always `null` and every session draws from the whole pool. An earlier StartScreen had a theme picker; it was dropped when the size/batch chip rows took over the screen. Leave the plumbing or wire a chip row back up — don't "clean up the dead code" without asking, it's a deliberately parked feature.
...
**New-words batch** — every `WordItem` carries a `batch: Int` (missing ⇒ Gson defaults 0). `GameViewModel` sets `newestBatch = max(batch)`; when it's > 0 the start screen shows the **new / all** selector (`hasNewBatch`), defaulting to the newest batch. So "new words" = the highest-numbered batch, "all words" = everything. **Footgun:** a child whose word file has **no `batch` on any row** (all default 0) gets `newestBatch = 0` and therefore **no selector at all** — this is the bug Roni hit after her words were bulk-dumped without batches (fixed by tagging her newer ids `batch: 1`). Batch 0 legitimately means "original set, not a new batch" (Oren's 78 bundled words), so the code can't auto-infer a new set — batches must be assigned in the data. **Always add words via the `add-words` skill** (`.claude/skills/add-words/SKILL.md`, committed to the repo — it assigns `batch = maxBatch + 1` and has a verify step); a raw JSON dump that skips `batch` silently kills the selector. The skill's upload step PUTs to JSONBin with an `X-Master-Key` header read from the **`JSONBIN_MASTER_KEY` environment variable** — never commit that key. Without it you can still edit the word files locally and paste them into the JSONBin web UI.
...
**Audio** — `util/SoundManager.kt` holds the `SoundManager` interface plus `AndroidSoundManager` (SoundPool) and `NoOpSoundManager` (the test/stub double injected via the `sound` seam). Clips live in `res/raw`: a **correct answer picks one of five** `success*.mp3` variants at random so repeat play doesn't grate, `fail.mp3` on a wrong answer, and `level_up.mp3` — kept from the retired leveling system and now reused as the **card-win** sting (fired from `checkAnswer` when `reconcileCards` awards a card). Every call is gated on `GameState.soundEnabled`, a **global** (not per-child) pref toggled in the settings dialog.
...
Asked for and declined, so don't add them speculatively: text-to-speech / word pronunciation, multiplayer, theme skins, a daily challenge, and spaced repetition. Persistence was scoped to best score + best streak per session-size bucket and nothing more. The `strings.xml` / localization question is settled the same way — the app is Hebrew-only on purpose.

Rejected designs worth not re-proposing: an `isNew: Boolean` flag instead of `batch: Int` (it mutates every old row on each new batch), and a top-level `newFromId` field (breaks the bare-array shape the JSONBin bins require).
